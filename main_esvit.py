...
def train_esvit(args):
    ...
    dataset_val = "/home/hl/xy/CUB_200_2011/dataset/val"
...
class DDINOLoss(nn.Module):
    ...
    def forward(self, student_output, teacher_output, epoch, targets_mixup):
        ...
        # print(f"F.log_softmax(s_cls[2], dim=-1)123333333{F.log_softmax(s_cls[2], dim=-1)}")
        # print(f"F.log_softmax(s_cls[0], dim=-1)123333333{F.log_softmax(s_cls[0], dim=-1)}")
        total_loss = 0
        n_loss_terms = 0
        # loss1 = torch.sum(F.softmax(s_cls[2], dim=-1)* F.log_softmax(s_cls[0], dim=-1), dim=-1)
        # loss2 = torch.sum(F.softmax(s_cls[3], dim=-1) * F.log_softmax(s_cls[0], dim=-1), dim=-1)
        # loss3 = torch.sum(F.softmax(s_cls[4], dim=-1) * F.log_softmax(s_cls[1], dim=-1), dim=-1)
        # loss4 = torch.sum(F.softmax(s_cls[5], dim=-1) * F.log_softmax(s_cls[1], dim=-1), dim=-1)
        # loss5 = 0.5*loss1 + 0.5*loss2 + 0.5*loss3 + 0.5*loss4
        loss1 = torch.sum(F.softmax(s_cls[2], dim=-1) * F.log_softmax(s_cls[1], dim=-1), dim=-1)
        loss2 = torch.sum(F.softmax(s_cls[2], dim=-1) * F.log_softmax(s_cls[3], dim=-1), dim=-1)
        loss3 = torch.sum(F.softmax(s_cls[2], dim=-1) * F.log_softmax(s_cls[4], dim=-1), dim=-1)
        loss4 = torch.sum(F.softmax(s_cls[2], dim=-1) * F.log_softmax(s_cls[5], dim=-1), dim=-1)
        loss5 = torch.sum(F.softmax(s_cls[2], dim=-1) * F.log_softmax(s_cls[6], dim=-1), dim=-1)
        loss6 = torch.sum(F.softmax(s_cls[2], dim=-1) * F.log_softmax(s_cls[7], dim=-1), dim=-1)
        loss7 = torch.sum(F.softmax(s_cls[2], dim=-1) * F.log_softmax(s_cls[8], dim=-1), dim=-1)
        loss8 = torch.sum(F.softmax(s_cls[2], dim=-1) * F.log_softmax(s_cls[9], dim=-1), dim=-1)
        loss111 = torch.sum(F.softmax(s_cls[1], dim=-1) * F.log_softmax(s_cls[0], dim=-1), dim=-1)
        loss222 = torch.sum(F.softmax(s_cls[0], dim=-1) * F.log_softmax(s_cls[1], dim=-1), dim=-1)
        loss555 = torch.sum(F.softmax(s_cls[1], dim=-1) * F.log_softmax(s_cls[2], dim=-1), dim=-1)
        loss666 = torch.sum(F.softmax(s_cls[0], dim=-1) * F.log_softmax(s_cls[3], dim=-1), dim=-1)
        loss777 = torch.sum(F.softmax(s_cls[1], dim=-1) * F.log_softmax(s_cls[3], dim=-1), dim=-1)
        loss888 = torch.sum(F.softmax(s_cls[0], dim=-1) * F.log_softmax(s_cls[2], dim=-1), dim=-1)
        # loss9 = torch.sum(F.softmax(s_cls[3], dim=-1) * F.log_softmax(s_cls[1], dim=-1), dim=-1)
        # loss10 = torch.sum(F.softmax(s_cls[3], dim=-1) * F.log_softmax(s_cls[2], dim=-1), dim=-1)
        # loss11 = torch.sum(F.softmax(s_cls[3], dim=-1) * F.log_softmax(s_cls[4], dim=-1), dim=-1)
        # loss12 = torch.sum(F.softmax(s_cls[3], dim=-1) * F.log_softmax(s_cls[5], dim=-1), dim=-1)
        # loss13 = torch.sum(F.softmax(s_cls[3], dim=-1) * F.log_softmax(s_cls[6], dim=-1), dim=-1)
        # loss14 = torch.sum(F.softmax(s_cls[3], dim=-1) * F.log_softmax(s_cls[7], dim=-1), dim=-1)
        # loss15 = torch.sum(F.softmax(s_cls[3], dim=-1) * F.log_softmax(s_cls[8], dim=-1), dim=-1)
        # loss16 = torch.sum(F.softmax(s_cls[3], dim=-1) * F.log_softmax(s_cls[9], dim=-1), dim=-1)


        loss17 = torch.sum(F.softmax(s_cls[3], dim=-1) * F.log_softmax(s_cls[0], dim=-1), dim=-1)
        loss18 = torch.sum(F.softmax(s_cls[3], dim=-1) * F.log_softmax(s_cls[2], dim=-1), dim=-1)
        loss19 = torch.sum(F.softmax(s_cls[3], dim=-1) * F.log_softmax(s_cls[4], dim=-1), dim=-1)
        loss20 = torch.sum(F.softmax(s_cls[3], dim=-1) * F.log_softmax(s_cls[5], dim=-1), dim=-1)
        loss21 = torch.sum(F.softmax(s_cls[3], dim=-1) * F.log_softmax(s_cls[6], dim=-1), dim=-1)
        loss22 = torch.sum(F.softmax(s_cls[3], dim=-1) * F.log_softmax(s_cls[7], dim=-1), dim=-1)
        loss23 = torch.sum(F.softmax(s_cls[3], dim=-1) * F.log_softmax(s_cls[8], dim=-1), dim=-1)
        loss24 = torch.sum(F.softmax(s_cls[3], dim=-1) * F.log_softmax(s_cls[9], dim=-1), dim=-1)

        # loss25 = torch.sum(F.softmax(s_cls[5], dim=-1) * F.log_softmax(s_cls[0], dim=-1), dim=-1)
        # loss26 = torch.sum(F.softmax(s_cls[5], dim=-1) * F.log_softmax(s_cls[4], dim=-1), dim=-1)
        # loss27 = torch.sum(F.softmax(s_cls[5], dim=-1) * F.log_softmax(s_cls[2], dim=-1), dim=-1)
        # loss28 = torch.sum(F.softmax(s_cls[5], dim=-1) * F.log_softmax(s_cls[3], dim=-1), dim=-1)
        # loss29 = torch.sum(F.softmax(s_cls[5], dim=-1) * F.log_softmax(s_cls[6], dim=-1), dim=-1)
        # loss30 = torch.sum(F.softmax(s_cls[5], dim=-1) * F.log_softmax(s_cls[7], dim=-1), dim=-1)
        # loss31 = torch.sum(F.softmax(s_cls[5], dim=-1) * F.log_softmax(s_cls[8], dim=-1), dim=-1)
        # loss32 = torch.sum(F.softmax(s_cls[5], dim=-1) * F.log_softmax(s_cls[9], dim=-1), dim=-1)

        # lossall = loss2 + loss3 + loss4 + loss5 + loss6 + loss7 + loss8  + loss18 + loss19 + loss20 + loss21 + loss22 + loss23 + loss24 
        # lossall = loss1+loss17+loss111+loss222+loss555+loss666
        # lossall = loss1+loss17+loss111+loss222+loss555+loss666+loss777+loss888
        lossall = loss1+loss17+1.5*loss111+1.5*loss222+1.5*loss555+1.5*loss666
        tt_cls1 = t_cls[0]
        # tt_cls2 = t_cls[0]
        tt_cls2 = t_cls[0]
        # tt_cls4 = t_cls[0]
        for iq, q in enumerate(t_cls):
            for v in range(len(s_cls)):
                ...
                # view part prediction loss
                # if (iq==1 and v>=64):
                #     loss1 = 0.5 * torch.sum(-q * F.log_softmax(s_cls[v], dim=-1), dim=-1)
                ...
                if((v==2 and iq==0) or (v==3 and iq==1)):
                    ...
                    # if(v==4):
                    #     tt_cls3=t_region_pooled_avgg1
                    # if(v==5):
                    #     tt_cls4=t_region_pooled_avgg1
        ...
        loss3333 = torch.sum(F.softmax(t_cls[1], dim=-1) * F.log_softmax(t_cls[0], dim=-1), dim=-1)
        loss4444 = torch.sum(F.softmax(t_cls[0], dim=-1) * F.log_softmax(t_cls[1], dim=-1), dim=-1)
        
        loss1111 = torch.sum(F.softmax(t_cls[1], dim=-1) * F.log_softmax(tt_cls1, dim=-1), dim=-1)
        loss2222 = torch.sum(F.softmax(t_cls[0], dim=-1) * F.log_softmax(tt_cls2, dim=-1), dim=-1)

        loss5555 = torch.sum(F.softmax(t_cls[1], dim=-1) * F.log_softmax(tt_cls1, dim=-1), dim=-1)
        loss6666 = torch.sum(F.softmax(t_cls[0], dim=-1) * F.log_softmax(tt_cls2, dim=-1), dim=-1)
        # loss33 = torch.sum(F.softmax(tt_cls3, dim=-1) * F.log_softmax(t_cls[0], dim=-1), dim=-1)
        # loss44 = torch.sum(F.softmax(tt_cls4, dim=-1) * F.log_softmax(t_cls[0], dim=-1), dim=-1)
        # losss = loss11+loss22+lossall+loss111+loss222+loss333+loss444
        losss = loss11+loss22+lossall+loss5555+loss6666+loss3333+loss4444
        # lossall = lossall+losss
        total_loss += 0.1*losss.mean()
        # n_loss_terms += 1

        # for iq, q in enumerate(t_cls_part):
        #     for v in range(len(s_cls)):
        #         if v <= 63:
        #             continue
        #         loss1 = 0.5 * torch.sum(-q * F.log_softmax(s_cls[v], dim=-1), dim=-1)
