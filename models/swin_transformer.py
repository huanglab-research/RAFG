...
def windowattn_reverse(windows, window_size, H, W):
    """
        Args:
        windows: (num_windows*B, window_size, window_size, C)
        window_size (int): Window size
...
def generate_2d_mask(H=16, W=8, left=0, top=0, width=8, height=8, device='cuda'):
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f"No9.att505050505····················:{shifted_x.size()}")
        # print(f"No3.att505050505····················:{x_windows.size()}")
        # W-MSA/SW-MSA
        attn_windows, attn = self.attn(x_windows, attn_mask)  # nW*B, window_size*window_size, C
        # print(f"No1.att78787878····················:{attn_windows.size()}")
        # print(f"No1.attn5656566····················:{attn.size()}")
        # attn = attn.mean(dim=1)
        res = torch.zeros(1,attn.size(0),attn.size(2),attn.size(3)).to(attn.device)
        attn = attn.permute(1,0,2,3)
        for i in range(len(attn)):
            res = res + attn[i]
        res = res/len(attn)
        res.squeeze(0)
        # print(f"No1.resssssssssss····················:{res}")
        # print(f"No1.resssssssssss····················:{res.size()}")
        # attn = attn.mean(dim=2)
        res = res.mean(dim=2)
        # print(f"No2.attn5656566····················:{attn}")
        # print(f"attn1shape:{attn.shape}")
        res = res.view(-1, self.window_size, self.window_size)
        # print(f"attn2shape:{attn.shape}")
        # merge windows
        attn_windows = attn_windows.view(-1, self.window_size, self.window_size, C)
        shifted_x = window_reverse(attn_windows, self.window_size, Hp, Wp)  # B H' W' C
        # print(f"x1shape:{shifted_x.shape}")
        shifted_attn = windowattn_reverse(res, self.window_size, Hp, Wp)
        # print(f"attn3shape:{shifted_attn.shape}")
        # reverse cyclic shift
        if self.shift_size > 0:
            x = torch.roll(shifted_x, shifts=(self.shift_size, self.shift_size), dims=(1, 2))
            # print(f"x2shape:{x.shape}")
            attn = torch.roll(shifted_attn, shifts=(self.shift_size, self.shift_size), dims=(1, 2))
            # print(f"attn4shape:{attn.shape}")
        else:
            x = shifted_x
            # print(f"x3shape:{x.shape}")
            attn = shifted_attn
            # print(f"attn5shape:{attn.shape}")
        if pad_r > 0 or pad_b > 0:
            x = x[:, :H, :W, :].contiguous()
            # print(f"x4shape:{x.shape}")
            attn = attn[:, :H, :W].contiguous()
            # print(f"attn6shape:{attn.shape}")
        x = x.view(B, H * W, C)
        # print(f"x5shape:{x.shape}")
        attn = attn.view(B, H * W)

        # print(f"attn5656566····················:{attn}")
        # FFN
...
class BasicLayer(nn.Module):
    ...
    def forward_with_features_attention(self, x):
        fea = []
        attns = []
        for blk in self.blocks:
            x, attn = blk(x)
...
class SwinTransformer(nn.Module):
    ...
    def batch_iou(self, boxesA, boxesB):
    
        xA = torch.max(boxesA[:, 0].unsqueeze(1), boxesB[:, 0].unsqueeze(0))  # [N, M]
        yA = torch.max(boxesA[:, 1].unsqueeze(1), boxesB[:, 1].unsqueeze(0))  # [N, M]
        xB = torch.min(boxesA[:, 2].unsqueeze(1), boxesB[:, 2].unsqueeze(0))  # [N, M]
        yB = torch.min(boxesA[:, 3].unsqueeze(1), boxesB[:, 3].unsqueeze(0))  # [N, M]

        inter_width = torch.max(xB - xA, torch.zeros_like(xB))  # 防止负数
        inter_height = torch.max(yB - yA, torch.zeros_like(yB))  # 防止负数
        intersection = inter_width * inter_height

        areaA = (boxesA[:, 2] - boxesA[:, 0]) * (boxesA[:, 3] - boxesA[:, 1])  # [N]
        areaB = (boxesB[:, 2] - boxesB[:, 0]) * (boxesB[:, 3] - boxesB[:, 1])  # [M]
    
        union = areaA.unsqueeze(1) + areaB.unsqueeze(0) - intersection  # [N, M]
        iou = intersection / union
        return iou



    def attn_mask_generate(self, N=132, H=16, W=8, left=0, top=0,width = 0, device='cuda'):
        # mask_ = torch.ones(N,1, device=device)
        # mask[1 : 4, 0] = 0
        # mask_ = (mask @ mask.t()).bool()
        mask_ = torch.zeros([H, W], device=device).bool()
        mask_ = generate_2d_mask(H,W,left,top,width,width, device).bool()
        ...
        return mask_
    def forward(self, x):
    # def forward(self, x, chh, r):
         # 遍历并打印每个键
        # for value in chh.values():
        #     print(f"试一下chh============= {value[0][0]}")

        # print(f"试一下chh===========11111== {len(chh)}")
        # print(f"试一下chh============= {chh.keys}")
    ...
    # def forward(self, x):
        ...
        # print(f"---66666666((((-{len(x)}")
        if len(x) == 12:

            # print(f"---------!!!@@@@@-{len(x)}")
        
            # last_element = torch.cat([x[-1],x[-1]])
            # duplicated_element = last_element.clone()
            # combined = torch.cat((last_element, duplicated_element), dim=0)
            # last_element = combined
            # print(f"---------!!!@@@@@-{last_element.shape}")
            x2 = x[-2]
            x1 = x[-1]
            x = x[:-2]
            print(f"lennnnnn12333333{len(x)}")
            print(f"lennnnnn12333333{x1.size()}")
            # print(f"---------!!!55555@@@@@-{len(x)}")
        ...
        if self.use_dense_prediction:
            ...
            # print(f"1Attn_map shape: {x[0].shape}")
            # print(f"2Attn_map shape: {x[1].shape}")
            # print(f"Attn_map shape: {len(x)}")
            # _f_cls, _f_fea  = self.forward_features(x[1])
            print(f"-------------------------")
            # mask_ = torch.zeros([32, 8, self.patch_embed.patches_resolution[0], self.patch_embed.patches_resolution[1]], device='cuda')
            masks_ = torch.zeros([32, 8, 7, 7], device='cuda')
            # mask1 = torch.zeros([32, 8, self.patch_embed.num_patches, self.patch_embed.num_patches], device='cuda')
            # mask_ = torch.cat((mask, mask1), dim=0)
            dic2 = {}
            for end_idx in idx_crops:
                # print(f"测试一下部分的{end_idx}和{start_idx}")
                attns = []
                h = [None] * 3
                testt = torch.cat(x[start_idx: end_idx])
                # print(f"````````````````测试一下部分的-------------{testt.size}")
                                    
                # mask_[j][i] = self.attn_mask_generate(self.patch_embed.num_patches,self.patch_embed.patches_resolution[0], self.patch_embed.patches_resolution[1], left,top ,18 , testt.device.type)
                # print(f"222Attn_map shape: {testt.shape}")
                _out_cls, _out_fea, attns = self.forward_features(testt)  #相同大小的裁剪进行合并
                
                # print(f" _out_fea---66666666``````````````((((-{_out_fea.size()}===={_out_cls.size()}")
                B, N, C = _out_fea.shape
                print(f" B---66666666``````````````((((-{B}====")
                ...
                #             for al in range(0,18):
                #                 for bl in range(0,18):
                                    
                #                     xx = (left+al)//8
                #                     yy = (top+bl)//8
                #                     masks_[j][i][xx][yy] = masks_[j][i][xx][yy]+1
                                   
                #             index = 0
                #             for xx in range(0,7):
                #                 for yy in range(0,7):
                #                     if masks_[j][i][xx][yy] >20:
                #                         clss[j][i] = clss[j][i]+_out_fea[j,3*yy+xx,:]
                #                         index = index+1
                #             clss[j][i] = clss[j][i]/index
                ...
                if(start_idx==0 and len(x)==10):
                    ...
                    # print(f"len(attns):{len(attns)}")
                    # print(f"attn不同层{attns[1]}")
                    ...
                    if h[-1].size(1) == 3136:
                        ...
                        B = h[2].size(0)
                        print(f"1233333333333333333iiiii{B}")
                        ...
                        # print(f"BBBBBBBBBBBBB333333333333333333")
                        for batchh in range(B):
                            ...
                            for column1 in range(0, 33, 3):
                                for row1 in range(0, 33, 3):
                                    ...
                                    # print(f"column:{column1}-------row{row1}")
                            sorted_s = sorted(s, key=lambda x: x[2], reverse=True)
                            p = [sorted_s[0]]

                            # print(f"sorted_s:-------------{column1}")
                            boxesA = torch.tensor([[x[0], x[1], x[0] + 23, x[1] + 23] for x in p], device='cuda', dtype=torch.float32)

                            # for i in range(1, 121):
                            #     if len(p) == 1:
                            #         break
                            #     boxB = torch.tensor([sorted_s[i][0], sorted_s[i][1], sorted_s[i][0] + 23, sorted_s[i][1] + 23], device='cuda', dtype=torch.float32).unsqueeze(0)

                            #     iou = self.batch_iou(boxesA, boxB)
                            #     if torch.all(iou <= 0.8):
                            #         p.append(sorted_s[i])
                            #         boxesA = torch.cat([boxesA, boxB], dim=0)
                            dic2[batchh] = p
                            dic1[batchh] = s
                        for o in range(0,64):
                            tt = []
                        ...
                        # print(f"1111111111111``````````````{dic2[0][0][0]}/////{dic2[0][0][1]}")
                        # print(f"1111111222222``````````````{dic2[0][1][0]}/////{dic2[0][1][1]}")
                        # print(f"222222222221111``````````````{dic2[32][2][0]}/////{dic2[32][2][1]}")
                        # print(f"222222222222222``````````````{dic2[32][3][0]}/////{dic2[32][3][1]}")

                        # print(f"1111111111111``````````````{dic3[0][0]}////")
                        # print(f"1111111222222``````````````{dic3[0][1]}////")
                        # print(f"222222222221111``````````````{dic3[32][0]}///")
                        # print(f"222222222222222``````````````{dic3[32][1]}///")
                    # if h[-1].size(1) == 3136:
                    ...
                    #     for batchh in range(0,32):
                    #         s = []
                    #         for column1 in range(0,39):
                    #             for row1 in range(0,39):
                                    
                    #                 summed_value = h[2][batchh,column1:column1+18,row1:row1+18].sum(dim=(0, 1))
                    ...
                    #                 boxBx1 = sorted_s[i][0]
                    #                 boxBy1 = sorted_s[i][1]
                    #                 boxBx2 = (sorted_s[i][0]+17)
                    #                 boxBy2 = (sorted_s[i][1]+17)
                    #                 xA = max(boxAx1, boxBx1)
                    #                 yA = max(boxAy1, boxBy1)
                    #                 xB = min(boxAx2, boxBx2)
                    #                 yB = min(boxAy2, boxBy2)
                    #                 interWidth = max(0, xB - xA)
                    #                 interHeight = max(0, yB - yA)
                    #                 intersection_area = interWidth * interHeight
                    #                 boxA_area = (boxAx2 -  boxAx1) * (boxAy2 - boxAy1)
                    #                 boxB_area = (boxBx2 - boxBx1) * (boxBy2 - boxBy1)
                    ...
                    #                 index = index+1
                            ...
                            #         transformss = transforms.Compose([
                            #             transforms.ToPILImage(),
                            #             CustomTransform(crop_x=p[r][0]*4, crop_y=p[r][1]*4, crop_width=72, crop_height=72),
                            #             # transforms.Resize((224, 224), interpolation=Image.BICUBIC),  # 三次插值
                            #             # transforms.RandomHorizontalFlip(p=0.5),  # 水平翻转
                            #             # transforms.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.2, hue=0.1),  # 颜色抖动
                            #             flip_and_color_jitter,
                            #             utils.GaussianBlur(p=0.5),
                            #             # normalize,
                            #             transforms.ToTensor(),
                            #             transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225)),
                            #         ]) 
                            #         # print(f"lllllll````````7876760^^^^^{x[2].shape}")
                            #         # print(f"lllllll````````90909090^^^^^{transformss(last_element[batchh]).shape}")
                            #         tt.append(transformss(last_element[batchh]))
                                    # print(f"lllllll````````90909090^^^^^{tt[r].shape}")

                                    # x[r+2] = transformss(last_element[batchh])
                                    # print(f"lllllll````````====5555565656565^^^^^{x[r+2].shape}")
                            # print(f"lllllll````````最后最后！！！！！{len(tt)}")
                            # dic1[batchh] = s
                            # dic2[batchh] = p
                            # for value in dic2.values():
                            
                            # dic3[batchh] = tt
                        # print(f"试一下dic22222222{len(dic2)}")

                        # dicc2 = dic2
                        # for hh in range(0,32):
                        #     for ll in range(2,10):
                                # print(f"lllllll````````7876760^^^^^{x[ll][hh].shape}")
                                # downsampled_x = F.interpolate(x[ll][hh].unsqueeze(0), size=(72, 72), mode='bicubic', align_corners=False).squeeze(0)
                                # x[ll][hh] = downsampled_x 
                                # print(f"lllllll````````333434343434^^^^^{x[ll][hh].shape}")
                                # x[ll][hh] = dic3[hh][ll-2]
                ...
                if start_idx == 0:   #如果是第一组那直接赋值
                    ...
                    # print(f"试一下ou_cls=============222222222 {output_cls.size()}")
                    output_fea = torch.cat((output_fea, _out_fea.reshape(B * N, C) ))
                    
                    # for i in range(1,7):
                    #     aa1 = clsss[0:index*32,i]
                    #     bb1 = clss[:,i]
                    #     print(f"试一下aa1=============222222222 {aa1.size()}")
                    #     print(f"试一下bb1=============222222222 {bb1.size()}")
                    #     clsss[0:(index+1)*32,i] = torch.cat((aa1,bb1))
                    # index = index+1
                    npatch.append(N) #为了知道每个原始数据段被划分成了多少个子部分
                start_idx = end_idx
                # for value in dic2.values():
                # print(f"试一下dicccccc=============222222222 {len(dic2)}")
            # print(f"试一下chh=============``````` !!!!!!!!!")
            if len(x) == 10:
                # print(f"111111111122222yuyuyuyuyuyu")
                # print(f"试一下chh=============``````` !!!2!!!!!{len(dic2)}")

                # for value in dic2.values():
                #     print(f"试一下chh=============``````` {value[0][0]}")
                # return self.head(output_cls), self.head_dense(output_fea), output_fea, npatch, dic2
                ...
                # for value in dic2.values():
                #     print(f"试一下chh=============``````` {value[0][0]}")
                # clsss = torch.zeros([256, 768], device='cuda')  
                # clsss = torch.cat((clss[:,0],clss[:,1],clss[:,2],clss[:,3],clss[:,4],clss[:,5],clss[:,6],clss[:,7]))
                return self.head(output_cls), self.head_dense(output_fea), output_fea, npatch
            # return self.head(output_cls), self.head_dense(output_fea), output_fea, npatch
